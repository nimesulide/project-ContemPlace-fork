...
npx vitest run tests/dashboard-smoke.test.ts    # Dashboard API Worker
```
...
npx tsc --noEmit -p dashboard-api/tsconfig.json   # Dashboard API Worker
```

## Deploy

```bash
bash scripts/deploy.sh          # Full: schema → typecheck → unit tests → MCP Worker → Telegram Worker → bot commands → Gardener Worker → Dashboard API Worker → Pages → smoke tests
...
wrangler deploy -c dashboard-api/wrangler.toml     # Dashboard API Worker
wrangler pages deploy dashboard/ --project-name contemplace-dashboard --branch main  # Pages frontend
```
...
wrangler dev -c dashboard-api/wrangler.toml    # Dashboard API Worker
```
...
dashboard-api/    Dashboard API Worker (read-only JSON API for the dashboard SPA)
  src/
    index.ts      Route dispatch — /stats, /clusters, /clusters/detail, /recent
    auth.ts       Bearer token auth
    config.ts     Config loading with secret validation
    db.ts         Read-only DB queries (stats, clusters, cluster detail, recent)
    github.ts     Backup freshness check via GitHub API
    types.ts      Env, response types (StatsResponse, ClusterCard, etc.)
  wrangler.toml
dashboard/        Cloudflare Pages SPA (vanilla HTML/CSS/ES-modules, no build step)
  index.html      App shell — import map for Cytoscape.js, CSS design tokens, three-panel layout
  js/
    app.js        Auth overlay, API client, panel initialization
    stats.js      Stats bar + health row panel
    clusters.js   Cluster grid + Cytoscape.js force-directed graph panel
    recent.js     Recent captures panel
  config.js       Generated by deploy.sh — sets window.CONTEMPLACE_API_URL (gitignored)
supabase/
...
tests/
  ...
  dashboard-api.test.ts        Dashboard API: auth, config, DB queries, routing, CORS
  dashboard-smoke.test.ts      Live Dashboard API Worker
.github/
  workflows/
