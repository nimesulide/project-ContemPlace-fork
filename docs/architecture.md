...
Cluster detection uses Louvain community detection via Graphology (pure JS, runs in CF Workers V8). Multi-resolution: runs at each value in `GARDENER_CLUSTER_RESOLUTIONS` (default 1.0, 1.5, 2.0). Higher resolution = more granular clusters. After community detection, clusters receive LLM-generated descriptive titles via a single batched call (all clusters in one prompt). This replaces the tag-based `"cooking / italian / pasta"` labels with natural-language titles like `"Laser-cut instruments, sound design, and electronic music fabrication"`. Title generation is gated on `OPENROUTER_API_KEY` — without it, the tag-based fallback is used. `top_tags` is always preserved regardless. Results stored in the `clusters` table with gravity (recency-weighted size), top-3 tags, and the generated label.
