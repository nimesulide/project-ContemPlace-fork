...
## Dashboard as a presentation layer (2026-03-20)
...
**Why:** The knowledge graph (clusters, links, similarity signals) had no visual surface. MCP tools expose data to agents; the dashboard exposes it to the human. Image capture (#209) confirmed the need — MCP clients can't render images inline, making the dashboard the only retrieval surface for visual notes. Specialist review confirmed: Cytoscape.js (best docs, native compound nodes, 112KB gzip), separate API Worker (deployment independence, separate secrets), three-panel layout (prior art shows full-graph is a demo feature), no framework (3 panels don't need React).

**Auth:** Bearer token in localStorage, not Cloudflare Access. Works across phone, tablet, desktop — paste key once per device. Access adds CORS complexity and setup overhead for no proportionate security gain in a single-user system. Can layer on later.

**Frontend modularity:** ES modules without build step. Each panel is a separate JS file with `init(api)` export. Import map for Cytoscape.js eliminates CDN script tag race conditions. API URL lives in a gitignored `config.js` generated by `deploy.sh` from `.dev.vars` — no URLs in the public repo.
...
## SYSTEM_FRAME body rules: no expansion + preserve uncertainty (2026-03-25)

**Decision:** Expanded the SYSTEM_FRAME body rule from a single question-preservation rule to three named sub-rules: Questions, No expansion, and Preserve uncertainty. Also added WRONG/RIGHT commitment-level counter-examples to the capture voice in `capture_profiles`.

**Context:** The 2026-03-25 capture audit (36 fragments, 2026-03-19 to 2026-03-25) found three systematic issues:

1. **Body contamination from related notes (3/36, trust contract violation):** Short inputs (<20 words, often photo captions) were expanded with content from related notes. "bird feeder with a piezo mic to turn pigeon pickings into music" (12 words) became a 30-word body containing details from a related note the user had captured hours earlier. The LLM treated brevity as incompleteness and filled in detail from available context.

2. **Hedging removal in body (2/36, trust contract violation):** "Maybe tool descriptions would work, but in my experience" became "Tool descriptions don't work in practice." The user's tentative observation was generalized into a definitive claim.

3. **Commitment level upgrade in titles (5/36, degrades retrieval):** Persistent since the 2026-03-15 audit. "thinking about getting a new bike" became "Research bikes and create moodboard." The existing capture voice rule wasn't sufficient — added concrete WRONG/RIGHT counter-examples.

**Why prompt rules, not pipeline validation:** All three issues are LLM behavioral drift, not structural bugs. Prompt-level fixes are cheap, immediately deployable, and testable via the next audit. A post-LLM validation step (comparing hedging markers in raw_input vs body/title) could catch these mechanically, but adds pipeline complexity. If the 2026-03-25 rules prove insufficient after the next audit, pipeline validation is the escalation path.

**Changes:**
- `mcp/src/capture.ts` SYSTEM_FRAME: body rule restructured into three named sub-rules (Questions, No expansion, Preserve uncertainty)
- `capture_profiles` DB: title section expanded with "thinking about" and "I have an idea about" as explicit hedging markers, plus three WRONG/RIGHT counter-examples and a vocabulary substitution example ("include the story" → "embed object provenance")
