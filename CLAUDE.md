...
| Dashboard | Cloudflare Pages (vanilla HTML/CSS/ES-modules SPA) + Dashboard API Worker (`contemplace-dashboard-api`) |
...
scripts/
  deploy.sh          # Automated deploy pipeline (schema → typecheck → unit tests → MCP Worker → Telegram Worker → bot commands → Gardener Worker → Dashboard API Worker → Pages → smoke tests)
...
dashboard-api/
  wrangler.toml      # Dashboard API Worker config (name: contemplace-dashboard-api)
  src/               # Route dispatch, auth, DB queries, GitHub backup freshness, response types
dashboard/
  index.html         # SPA shell — Cytoscape.js import map, CSS design tokens, three-panel layout
  js/                # ES-module panels: app.js, stats.js, clusters.js, recent.js
  config.js          # Generated by deploy.sh from .dev.vars — gitignored, sets window.CONTEMPLACE_API_URL
supabase/
  config.toml
...
Four Workers, each with their own secrets scope:
...
- **Dashboard API Worker** (set via `-c dashboard-api/wrangler.toml`) — `DASHBOARD_API_KEY`, `GITHUB_BACKUP_PAT` (optional), plus `SUPABASE_*` keys. Dashboard URL and backup repo in `wrangler.toml [vars]`.
...
```bash
# Deploy everything (schema → typecheck → tests → all 4 Workers + Pages → smoke tests)
